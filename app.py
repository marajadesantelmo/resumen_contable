...
def show_page(): 
    ...
    if not st.session_state.logged_in:
        ...
        with col_title:
            ...
            st.dataframe(resumen_contable_total, use_container_width=True, hide_index=True, disabled=True)
        
        st.write("Detalle por Sociedad")
        st.dataframe(resumen_contable, use_container_width=True, hide_index=True, disabled=True)
        ...
        with col1:
            ...
            st.dataframe(filtered_emitidos_por_empresa, use_container_width=True, hide_index=True, disabled=True)
        with col2:
            st.subheader("Recibidos por Empresa")
            st.dataframe(filtered_recibidos_por_empresa, use_container_width=True, hide_index=True, disabled=True)
        
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Detalle Comprobantes Emitidos")
            st.dataframe(filtered_emitidos, use_container_width=True, hide_index=True, disabled=True)
        with col2:
            st.subheader("Detalle Comprobantes Recibidos")
            st.dataframe(filtered_recibidos, use_container_width=True, hide_index=True, disabled=True)
